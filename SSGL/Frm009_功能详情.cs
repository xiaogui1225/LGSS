...
namespace SSGL
{
    public partial class Frm009_功能详情 : Form
    {
        ...
        public Frm009_功能详情(Frm001_主界面 frm001_主界面)
        {
            this.frm001_主界面 = frm001_主界面;
        }

        private void Frm09_功能详情_Load(object sender, EventArgs e)
        {
            //string sqlstr = SQLstr.sqlstr;
            //string sql, sqlsele, sqlins;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (comboBox1.Text == "投诉")
            {
                ...
                {
                    string sqlstr = SQLstr.sqlstr;
                    string sqlins = "insert into  Complaint values('" + textBox1.Text + "','" + textBox2.Text.Trim() + "','" + textBox3.Text.Trim() + "','" + textBox4.Text.Trim() + "','" + textBox5.Text.Trim() + "','" + textBox6.Text.Trim() + "','" + textBox7.Text.Trim() + "')";
                    SqlConnection con = new SqlConnection(sqlstr);
                    con.Open();
            ...
            if (comboBox1.Text=="申请换宿舍")
            {
                ...
                {
                    string sqlstr = SQLstr.sqlstr;
                    string sqlsele = "select * from changedor where sno='" + textBox1.Text.Trim() + "'";
                    SqlConnection con = new SqlConnection(sqlstr);
                    con.Open();
                    SqlCommand cmd = new SqlCommand(sqlsele, con);
                    SqlDataReader dr = cmd.ExecuteReader();
                    if (dr.HasRows)//如果没有申请过就可以申请，没有申请过可以申请
                    {
                        dr.Read();
                        string sno = dr["sno"].ToString().Trim();
                        if (textBox1.Text == sno)
                        { MessageBox.Show("一个人只有一次机会申请哦");
                            dr.Close();
                            con.Close();
                        }
                    }
